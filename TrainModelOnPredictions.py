...
def TrainModelOnPredictions(parameters, parameters2, use_best_model=False):
    ...
    try:
        model = keras.models.load_model('output/'+tag+'/ModelsOnPredictions/output/'+tag2+'/model.h5')
        train_new_model = False
    except:
        pass
    if train_new_model: print 'Couldn\'t find the model "model_%s", a new one will be trained!' % ('output/'+tag+'/ModelsOnPredictions/output/'+tag2+'/model.h5')
    else:
        print 'Found the model, not training a new one, go on to next function.'
        return
        # print 'Found model, but I will retrain it!'
    if not os.path.isdir('output/'+tag+'/ModelsOnPredictions/output/'+tag2): os.makedirs('output/'+tag+'/ModelsOnPredictions/output/'+tag2)
    ...
    input_train, input_test, input_val, signals = load_predictions(outputfolder='output/'+tag, filepostfix=postfix)
    # pred_train, pred_test, pred_val, pred_signals = load_predictions(outputfolder='output/'+tag, filepostfix=postfix)
    ...
    period = epochs / 5
    checkpointer = ModelCheckpoint(filepath='output/'+tag+'/ModelsOnPredictions/output/'+tag2+'/model_epoch{epoch:02d}.h5', verbose=1, save_best_only=False, period=period)
    checkpoint_bestmodel = ModelCheckpoint(filepath='output/'+tag+'/ModelsOnPredictions/output/'+tag2+'/model_best.h5', monitor='val_loss', verbose=0, save_best_only=True, save_weights_only=False, mode='min', period=1)
    ...
    model.save('output/'+tag+'/ModelsOnPredictions/output/'+tag2+'/model.h5')
    with open('output/'+tag+'/ModelsOnPredictions/output/'+tag2+'/model_history.pkl', 'w') as f:
        pickle.dump(model.history.history, f)


    PredictExternalOnPredictions(parameters, inputfolder='input/'+classtag, inputfolder_predictions='output/'+tag, filepostfix=postfix, outputfolder='output/'+tag+'/ModelsOnPredictions/output/'+tag2)
